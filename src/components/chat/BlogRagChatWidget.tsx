...
export function BlogRagChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      sender: 'bot',
      text: 'Bonjour. Je suis l\'assistant RAG du blog. Posez-moi une question sur mes articles et publications.'
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const userMsg: Message = { id: Date.now().toString(), sender: 'user', text: question.trim() };
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), sender: 'bot', text: `Erreur: ${err.message}` }
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 150, fontFamily: 'var(--font-sans)' }}>
      {!isOpen ? (
        ...
        >
          ...
          <div style={{ textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
                Assistant RAG
              </span>
              <span style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: '#10b981', display: 'inline-block' }} />
            </div>
            <p style={{ fontSize: '11px', color: 'var(--color-text-muted)', margin: 0, lineHeight: 1.2 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <h3 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text)', margin: 0, letterSpacing: '-0.01em' }}>
                    Assistant RAG
                  </h3>
                  <span
                    style={{
          ...
          >
              ...
              >
                ...
                <div
                  style={{
                    maxWidth: '90%',
                    backgroundColor: 'var(--color-surface)',
                    border: msg.sender === 'user' ? '1px solid var(--color-teal)' : '1px solid var(--color-border)',
                    borderRadius: 4,
                    padding: '12px 14px',
                  ...
                  {msg.sender === 'bot' && msg.id !== 'welcome' && (
                    <button
                      ...
                      style={{
                        ...
                        background: 'transparent',
                        border: 'none',
                        color: isSpeaking ? 'var(--color-teal)' : 'var(--color-text-muted)',
                        fontFamily: 'var(--font-mono)',
                        fontSize: 11,
                        cursor: 'pointer',
                        display: 'flex',
                      ...
                      onMouseLeave={(e) => (e.currentTarget.style.color = isSpeaking ? 'var(--color-teal)' : 'var(--color-text-muted)')}
                    >
                      <Volume2 size={13} style={{ color: isSpeaking ? 'var(--color-teal)' : 'inherit' }} />
                    ...
                    <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--color-border)' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {msg.citations.map((c, i) => (
                          ...
                          >
                            <span style={{ fontWeight: 550, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', paddingRight: 8 }}>
                              {c.title}
                            </span>
            ...
            {isLoading && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--color-text-muted)', marginBottom: 4 }}>
                  Assistant RAG
                </span>
                <div
                  style={{
                ...
                >
                  <Loader2 size={14} style={{ animation: 'spin 1s linear infinite', color: 'var(--color-teal)' }} />
          ...
          >
            <button
              ...
              style={{
                ...
                height: 36,
                borderRadius: 4,
                border: isListening ? '1px solid #ef4444' : '1px solid var(--color-border)',
                backgroundColor: isListening ? 'rgba(239, 68, 68, 0.1)' : 'var(--color-bg)',
            ...
            <input
              ...
              placeholder={isListening ? 'Écoute vocale...' : 'Posez une question sur les articles...'}
              style={{
                flex: 1,
                backgroundColor: 'var(--color-bg)',
                color: 'var(--color-text)',
